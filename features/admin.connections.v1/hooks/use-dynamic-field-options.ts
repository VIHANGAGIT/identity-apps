 ...
 */
export enum DynamicFieldOptionsSourceTypes {
    /**
 ...
 *
 * Declared in a connector's `metadata.json` next to the field it belongs to, e.g.
 * ```json
 * {
 *     "type": "select",
 *     "name": "daon_idp_id",
 *     "optionsSource": {
 *         "type": "connections",
 *         "authenticatorId": "RGFvbkF1dGhlbnRpY2F0b3I",
 *         "templateId": "daon-idv",
 *         "excludeCurrent": true
 *     }
 * }
 * ```
 */
export interface DynamicFieldOptionsSourceInterface {
    /**
     ...
     *
     * @remarks The connections list response does not carry the template id, so this filter costs
     * one additional request per candidate resolved by {@link authenticatorId}. Always pair it with
     * `authenticatorId` to keep the candidate set small.
     */
    templateId?: string;
    /**
 ...
 */
export interface DynamicFieldOptionsContextInterface {
    /**
     * Resource id of the connection being edited, if any.
     */
    currentConnectionId?: string;
    /**
     * Values the form was initialized with, keyed by field name. Used to keep an already persisted
     * value selectable even when it is not part of the resolved options.
     */
 ...
 */
export interface DynamicFieldOptionsResultInterface {
    /**
...
const useDynamicFieldOptions = (
     ...
     *
     * @param source - Options source of a field.
     *
    ...
    /**
     * Resolves the template id of every candidate. A candidate whose details cannot be read is kept
     * rather than dropped, so a failing request never hides an otherwise valid connection.
     */
    useEffect(() => {
        ...
        return fields.map((field: Record<string, any>) => {
            ...
            const options: { text: string; value: string }[] = getCandidates(source)
                .filter((connection: StrictConnectionInterface) => {
                    ...
                    // Keep the connection when its template id could not be resolved.
                    return !resolvedTemplateId || resolvedTemplateId === source.templateId;
                })
            ...
            /*
             * Keep an already persisted value selectable even when it is not part of the resolved
             * options - the referenced connection may have been deleted or may no longer match the
             * filters. Without this, saving an unrelated field would silently clear the reference.
             */
            ...
            if (!isEmpty(persistedValue)
                ...
                options.push({
                    // While the options are being resolved the value is shown as is, so the field is
                    // never rendered with a value that is not one of its options.
                    text: isLoading
                        ? persistedValue
